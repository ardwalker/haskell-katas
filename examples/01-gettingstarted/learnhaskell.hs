...
-- Where bindings are a syntactic construct that let you bind to variables at the end of a function and the whole function can see them, including all the guards. It is important to know that let ... in ... is an expression, that is, it can be written wherever expressions are allowed. 


-- Let bindings let you bind to variables anywhere and are expressions themselves, but are very local, so they don't span across guards. In contrast, where is bound to a surrounding syntactic construct, like the pattern matching line of a function definition.
...
-- Read on its own won't work, because it doesnt know the type, so specify it
-- ghci> read "5" :: Int
-- 5
...
-- is, x = 3 should not be read as “x gets 3” or “assign 3 to x”, but as “x is
-- defined to be 3”.
x :: Int
x = 3
main = print x
...
-- Integer types are very large, limited by amount of memory in your machine
reallyBig :: Integer
reallyBig = 2^(2^(2^(2^2)))
...
-- Now try this:
i = 30 :: Int
main = print (i / i) -- Error since / performs floating-point division only 

-- For integer division we can use div, either prefix or infix
i = 30 :: Int
main = print (i `div` i, div 12 5)
...
-- This declares a new type called Thing with five data constructors Shoe,
-- Ship, etc which are the (only) values of type Thing. [The deriving Show is a
-- magical incantation which tells GHC to automatically generate default code
-- for converting Things to Strings. This is what GHCi uses when printing the
-- value of an expression of type Thing.]
data Thing = Shoe
           | Ship
           | SealingWax
           | Cabbage
...
shoe :: Thing  -- shoe has type Thing 
shoe = Shoe    -- shoe is defined as Shoe

listO'Things :: [Thing]
listO'Things = [Shoe, SealingWax, King, Cabbage, King]  


-- This is another Enumeration type, but with a constructor which takes Double
-- /show
